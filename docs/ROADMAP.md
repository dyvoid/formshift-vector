...
| Palette control (pin/add/remove colors) | Done | Auto-generated palette is a proposal; the user customizes it — edit/add (eyedropper via the browser EyeDropper API)/remove swatches — and re-runs against the explicit palette (nearest-color, no clustering). Landed 2026-07-19 (`task/palette-seam-control`) on the server's explicit-`palette` param (server ADR 0020); client swatch editor (`PaletteEditor.tsx`) rides the existing two-phase flow, UI capped at 32 entries. Foundation for M4 spot-color separation. Verified live | — |
| Seam overlap / trapping (colormask grow) | Done | Disjoint per-color masks left hairline gaps at shared boundaries. `image.colormask` `grow` param (server ADR 0021) dilates each mask N px so adjacent traces overlap and cover the seam; client exposes a "Seam overlap (px)" slider (0–4, default 1) in the Quantize fieldset. Landed 2026-07-19 (`task/palette-seam-control`) | — |
