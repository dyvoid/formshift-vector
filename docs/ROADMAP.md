...
| Palette control (pin/add/remove colors) | Planned | Frequency-based quantizers drop small-but-important regions (e.g. eyes at 32 colors) — no automatic algorithm can know they matter. Auto-generate the palette as a proposal, then let the user edit it: pin/add (eyedropper on source)/remove/merge swatches; quantize against the explicit palette (nearest-color, no clustering). Server-first: `image.posterize` gains an optional explicit `palette` param; client adds the swatch editor + eyedropper on the existing two-phase flow (phase 1 proposes, user edits, phase 2 commits). Also the natural foundation for M4 spot-color separation (ink colors are chosen, not emergent). Server may also upgrade the default quantizer (libimagequant-class) as a low-effort quality win — helps gradients, doesn't replace user control | — |
